...
function executionCost(q, p) {
  const size3 = Math.abs(q);
  if (size3 === 0)
    return 0;
  const spread = p.spreadBps / 1e4 * size3;
  const impact = p.depth > 0 ? p.etaImpact * size3 * Math.sqrt(size3 / p.depth) : 0;
...
function frobeniusNormSq(a) {
  return frobeniusInner(a, a);
}
function quadForm(w, a) {
  let s = 0;
...
function ledoitWolf(x) {
  const T = x.length;
  if (T < 2)
    throw new RangeError("au moins deux observations sont requises");
  const n = x[0].length;
  const means = new Array(n).fill(0);
  for (const row of x)
    for (let j = 0;j < n; j++)
      means[j] = means[j] + row[j] / T;
  const c = x.map((row) => row.map((v, j) => v - means[j]));
  const sample = zeros(n);
  for (const row of c) {
    for (let i2 = 0;i2 < n; i2++) {
      for (let j = 0;j < n; j++)
        sample[i2][j] = sample[i2][j] + row[i2] * row[j] / T;
    }
  }
  const m = trace(sample) / n;
  const target = identity(n).map((row) => row.map((v) => v * m));
  const d2 = frobeniusNormSq(sample.map((row, i2) => row.map((v, j) => v - target[i2][j])));
  let b2bar = 0;
  for (const row of c) {
    let acc = 0;
    for (let i2 = 0;i2 < n; i2++) {
      for (let j = 0;j < n; j++) {
        const d = row[i2] * row[j] - sample[i2][j];
        acc += d * d;
      }
    }
    b2bar += acc / n;
  ...
  for (let i2 = 0;i2 < n; i2++) {
    for (let j = 0;j < n; j++) {
      sigma[i2][j] = intensity * target[i2][j] + (1 - intensity) * sample[i2][j];
    }
  }
  for (let i2 = 0;i2 < n; i2++) {
    for (let j = i2 + 1;j < n; j++) {
      const avg = (sigma[i2][j] + sigma[j][i2]) / 2;
      sigma[i2][j] = avg;
      sigma[j][i2] = avg;
    }
  }
...
function conditionalCovariance(residuals, currentVol) {
  const n = currentVol.length;
  const { sigma: sz, intensity } = ledoitWolf(residuals);
  const correlation = zeros(n);
  for (let i2 = 0;i2 < n; i2++) {
    for (let j = 0;j < n; j++) {
      correlation[i2][j] = sz[i2][j] / Math.sqrt(sz[i2][i2] * sz[j][j]);
    }
  }
  const sigma = zeros(n);
  for (let i2 = 0;i2 < n; i2++) {
    for (let j = 0;j < n; j++) {
      sigma[i2][j] = currentVol[i2] * correlation[i2][j] * currentVol[j];
    }
  }
  return { sigma, correlation, intensity };
}
var Z_99 = 2.3263478740408408;
function portfolioSigma(weights, sigma) {
  const v = quadForm(weights, sigma);
  return Math.sqrt(Math.max(v, 0));
}
function normalVaR(sigmaP, horizonDays, z = Z_99) {
  return z * sigmaP * Math.sqrt(horizonDays);
}
function filteredHistoricalES(input) {
  ...
  for (const z of residuals) {
    ...
    for (let i2 = 0;i2 < weights.length; i2++) {
      pnl += weights[i2] * currentVol[i2] * z[i2] * scale;
    }
    losses.push(-pnl);
  }
  losses.sort((a, b) => b - a);
...
function quantize(amount, lot) {
  if (lot <= 0)
    return amount;
  return Math.floor(amount / lot) * lot;
}
function canonicalizeOrders(orders) {
  return orders.map((o2) => `${o2.sell}>${o2.buy}:${o2.amount}`).sort().join("|");
}
function decide(input) {
  ...
  const available = {};
  for (const c of [numeraire, ...currencies])
    available[c] = balances[c] ?? 0;
  ...
  for (const c of currencies) {
    ...
    if (!policy)
      continue;
    const b = available[c];
    if (b < policy.bands.lower)
      raw.push({ currency: c, delta: policy.bands.target - b });
    else if (b > policy.bands.upper)
      raw.push({ currency: c, delta: policy.bands.target - b });
  }
  let candidates = raw.map(({ currency, delta }) => {
  ...
  }).filter((o2) => Math.abs(o2.signed) >= risk.minOrder);
  const buys = candidates.filter((o2) => o2.signed > 0);
  const sells = candidates.filter((o2) => o2.signed < 0);
  const proceeds = sells.reduce((a, o2) => a - o2.signed, 0);
  const needed = buys.reduce((a, o2) => a + o2.signed, 0);
  const spendable = Math.max(0, (available[numeraire] ?? 0) + proceeds - risk.fundingFloor);
  let fundingScale = 1;
  if (needed > spendable && needed > 0)
    fundingScale = spendable / needed;
  const grossAfterFunding = needed * fundingScale + proceeds;
  const epochScale = grossAfterFunding > risk.maxPerEpoch && grossAfterFunding > 0 ? risk.maxPerEpoch / grossAfterFunding : 1;
  candidates = candidates.map((o2) => ({
    currency: o2.currency,
    signed: quantize(Math.abs(o2.signed) * (o2.signed > 0 ? fundingScale : 1) * epochScale, risk.lotSize) * Math.sign(o2.signed)
  })).filter((o2) => Math.abs(o2.signed) >= risk.minOrder);
  const orders = candidates.map((o2) => o2.signed > 0 ? { sell: numeraire, buy: o2.currency, amount: o2.signed } : { sell: o2.currency, buy: numeraire, amount: -o2.signed });
  const before = currencies.map((c) => available[c] ?? 0);
  const after = before.slice();
  candidates.forEach((o2) => {
    const i2 = currencies.indexOf(o2.currency);
    if (i2 >= 0)
      after[i2] = after[i2] + o2.signed;
  });
  const { sigma } = conditionalCovariance(input.residuals, input.currentVol);
  const grossBefore = before.reduce((a, x) => a + Math.abs(x), 0);
  const toBps = (value2, base) => base > 0 ? Math.round(value2 / base * 1e4) : 0;
  const esOf = (w, horizonDays) => filteredHistoricalES({
  ...
  for (const o2 of candidates) {
    const policy = policies[o2.currency];
    const leg = currencies.map((c) => c === o2.currency ? Math.abs(o2.signed) : 0);
    settlementRisk += esOf(leg, Math.max(policy.settlementDays, risk.horizonDays));
  }
  let costEstimate = 0;
  for (const o2 of candidates) {
    const policy = policies[o2.currency];
    costEstimate += policy.costs.gammaFixed + executionCost(o2.signed, policy.costs);
  }
  const gross = orders.reduce((a, o2) => a + o2.amount, 0);
  const metrics = {
  ...
  return {
    status: "PROPOSE",
    reason: fundingScale < 1 ? "plan réduit par la contrainte de financement" : epochScale < 1 ? "plan réduit par le plafond de notionnel de l’epoch" : "écart aux bandes",
    orders,
...
function keccakF(a) {
  for (let round = 0;round < 24; round++) {
    ...
    for (let x2 = 0;x2 < 5; x2++) {
      c[x2] = a[x2] ^ a[x2 + 5] ^ a[x2 + 10] ^ a[x2 + 15] ^ a[x2 + 20];
    }
    for (let x2 = 0;x2 < 5; x2++) {
      const d = c[(x2 + 4) % 5] ^ rotl(c[(x2 + 1) % 5], 1);
      for (let y2 = 0;y2 < 25; y2 += 5)
        a[x2 + y2] = a[x2 + y2] ^ d;
    }
    let x = 1;
    let y = 0;
  ...
  for (let offset = 0;offset < padded.length; offset += RATE_BYTES) {
    for (let lane = 0;lane < RATE_BYTES / 8; lane++) {
      ...
      for (let byte = 7;byte >= 0; byte--) {
        value2 = value2 << 8n | BigInt(padded[offset + lane * 8 + byte]);
      }
      state[lane] = state[lane] ^ value2;
    }
    keccakF(state);
  }
  const out = new Uint8Array(32);
  for (let lane = 0;lane < 4; lane++) {
    let value2 = state[lane];
    for (let byte = 0;byte < 8; byte++) {
      out[lane * 8 + byte] = Number(value2 & 0xffn);
      value2 >>= 8n;
    }
  }
  return out;
...
function fromHex2(hex) {
  ...
  if (clean3.length % 2 !== 0)
    throw new RangeError("chaîne hexadécimale de longueur impaire");
  const out = new Uint8Array(clean3.length / 2);
  for (let i2 = 0;i2 < out.length; i2++)
    out[i2] = Number.parseInt(clean3.substr(i2 * 2, 2), 16);
  return out;
}
var WORD = 32;
var TWO_POW_256 = 1n << 256n;
function word(value2) {
  const v = value2 < 0n ? TWO_POW_256 + value2 : value2;
  if (v >= TWO_POW_256)
    throw new RangeError(`valeur hors capacité d'un mot : ${value2}`);
  const out = new Uint8Array(WORD);
  let rest = v;
  for (let i2 = WORD - 1;i2 >= 0; i2--) {
    out[i2] = Number(rest & 0xffn);
    rest >>= 8n;
  }
  return out;
}
function addressWord(address2) {
  const bytes = fromHex2(address2);
  if (bytes.length !== 20)
    throw new RangeError(`adresse invalide : ${address2}`);
  const out = new Uint8Array(WORD);
  out.set(bytes, WORD - 20);
  return out;
}
function bytes32Word(hex) {
  const bytes = fromHex2(hex);
  if (bytes.length !== 32)
    throw new RangeError(`bytes32 invalide : ${hex}`);
  return bytes;
}
...
function encodeOrdersAndSalt(orders, salt) {
  const head = [word(0x40n), bytes32Word(salt)];
  const body = [word(BigInt(orders.length))];
  for (const o2 of orders) {
    body.push(addressWord(o2.sell), addressWord(o2.buy), word(o2.amountIn), word(o2.minAmountOut));
  }
  return concat2([...head, ...body]);
}
function ordersCommitment(orders, salt) {
  return toHex2(keccak2562(encodeOrdersAndSalt(orders, salt)));
}
var REPORT_TYPE_STRING = "RebalanceReport(uint64 epoch,uint64 nonce,uint64 expiry,uint64 inputsTimestamp," + "uint32 policyVersion,bytes32 bandParamsHash,bytes32 inputsHash," + "bytes32 ordersCommitment,int32 esBeforeBps,int32 esAfterBps,uint128 costEstimate," + "uint128 grossNotional)";
var encoder3 = new TextEncoder;
var encoder4 = new TextEncoder;
function stableHash(value2) {
  const canonical = JSON.stringify(value2, (_key, v) => {
    if (v && typeof v === "object" && !Array.isArray(v)) {
      return Object.fromEntries(Object.entries(v).sort());
    }
    return v;
  });
  return toHex2(keccak2562(encoder4.encode(canonical)));
}
function deriveSalt(seed, epoch, nonce) {
  return toHex2(keccak2562(encoder4.encode(`${seed}|${epoch}|${nonce}`)));
}
...
function buildReport(input) {
  ...
  const decisionInput = {
    ...
    policies: Object.fromEntries(chain.currencies.map((c) => [c, { ...treasury.limits[c], bands: treasury.bands[c] }])),
    risk: treasury.risk,
  ...
  const orders = [];
  for (const o2 of decision.orders) {
    const buysForeign = o2.sell === chain.numeraire;
    const foreign = buysForeign ? o2.buy : o2.sell;
    const rate = market.rates[foreign];
    if (rate === undefined || rate <= 0)
      throw new RangeError(`taux manquant pour ${foreign}`);
    const keep = (1e4 - chain.slippageBps) / 1e4;
    const amountIn = buysForeign ? o2.amount : o2.amount * rate;
    const grossOut = buysForeign ? o2.amount * rate : o2.amount;
    orders.push({
      sell: chain.tokens[o2.sell],
      buy: chain.tokens[o2.buy],
      amountIn: toUnits(amountIn, chain.decimals),
      minAmountOut: toUnits(grossOut * keep, chain.decimals)
    });
  }
  const grossNotional = orders.reduce((a, o2) => a + (o2.sell === chain.tokens[chain.numeraire] ? o2.amountIn : o2.minAmountOut), 0n);
  const onchainOrders = orders.map((o2) => ({
    sell: o2.sell,
    buy: o2.buy,
    amountIn: o2.amountIn,
    minAmountOut: o2.minAmountOut
  }));
  const commitment = orders.length > 0 ? ordersCommitment(onchainOrders, salt) : `0x${"00".repeat(32)}`;
  return {
...
var configSchema = exports_external.object({
  ...
  treasuryUrl: exports_external.string(),
  marketUrl: exports_external.string(),
  treasuryTokenSecretId: exports_external.string(),
  saltSeedSecretId: exports_external.string(),
  numeraire: exports_external.string(),
  currencies: exports_external.array(exports_external.string()),
  tokens: exports_external.record(exports_external.string()),
  decimals: exports_external.number(),
  slippageBps: exports_external.number(),
  validitySec: exports_external.number()
});
var httpGet = (runtime2, url, bearer) => {
  const response = new cre.capabilities.HTTPClient().sendRequest(runtime2, {
    url,
    method: "GET",
    ...bearer ? { multiHeaders: { Authorization: { values: [`Bearer ${bearer}`] } } } : {}
  }).result();
  if (!ok(response)) {
...
var onCronTrigger = (runtime2) => {
  ...
  const treasuryToken = runtime2.getSecret({ id: config.treasuryTokenSecretId }).result().value;
  const saltSeed = runtime2.getSecret({ id: config.saltSeedSecretId }).result().value;
  const treasury = JSON.parse(httpGet(runtime2, config.treasuryUrl, treasuryToken));
  const market = JSON.parse(httpGet(runtime2, config.marketUrl));
  const chain = {
  ...
  runtime2.log(`décision : ${outcome.status} — ${outcome.reason}`);
  if (outcome.status !== "PROPOSE") {
    return `${outcome.status} : ${outcome.reason}`;
  }
  const donRuntime = runtime2.usingTheDons();
  const r = outcome.report;
  const encodedPayload = encodeAbiParameters(parseAbiParameters("uint64 epoch, uint64 nonce, uint64 expiry, uint64 inputsTimestamp," + "uint32 policyVersion, bytes32 bandParamsHash, bytes32 inputsHash," + "bytes32 ordersCommitment, int32 esBeforeBps, int32 esAfterBps," + "uint128 costEstimate, uint128 grossNotional"), [
    r.epoch,
    r.nonce,
    r.expiry,
    r.inputsTimestamp,
